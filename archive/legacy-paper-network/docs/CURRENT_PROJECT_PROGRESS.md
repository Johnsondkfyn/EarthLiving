...
- ArchitectModule `0.2.5` is live on hub/test and Main with the first working NPC-only Constructor bridge. Hub testing on 2026-05-31 confirmed the intended flow: create/select a Citizens NPC with `/architect builder <npc-id>`, generate a blueprint, position and rotate the client-side ghost with `/architect preview <id> look`, then left-click to submit the placement as a Constructor build order. The NPC visibly walks the site and places blocks slowly. ArchitectModule exports rotated Constructor copies under `plugins/Constructor/schematics/architect/`. The old Constructor `3.5` jar remains unchanged. Main loaded `0.2.5` cleanly and is ready for its first builder NPC.

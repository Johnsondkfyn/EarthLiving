...
- EarthLivingCore `0.7.4` is deployed on the hub test server with a clearer placement preview command: `/el preview <width> <height> <depth> [yOffset] [seconds]`. It draws a dense temporary particle box with floor grid, stronger edges, corner markers and an origin marker so schematics can be height/footprint checked before Constructor or WorldEdit placement.

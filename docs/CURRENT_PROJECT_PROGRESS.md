...
- ArchitectModule `0.1.0` has been implemented in the repository as an admin-only blueprint/schematic generator. It provides `/architect search`, `/architect generate`, `/architect preview`, `/architect paste`, `/architect list` and `/architect reload`, writes `.schem` files through WorldEdit, keeps web/AI lookup optional and disabled by default, and should be tested on the hub before any Main deployment.

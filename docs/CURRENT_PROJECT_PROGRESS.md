...
- ArchitectModule `0.1.0` has been implemented in the repository and deployed to the hub/test server as an admin-only blueprint/schematic generator. It provides `/architect search`, `/architect generate`, `/architect preview`, `/architect preview <id> look`, `/architect paste`, `/architect paste <id> look`, `/architect cancel`, `/architect list` and `/architect reload`, writes `.schem` files through WorldEdit, keeps web/AI lookup optional and disabled by default, and loaded cleanly with WorldEdit, Citizens and Constructor on the hub. The visual preview follows the block the admin is looking at, mouse wheel/hotbar scroll rotates 90 degrees at a time, left-click places the schematic, and the preview renderer now reuses overlapping client-side block changes to reduce flicker while looking around. Main deployment should wait until this full preview/rotation/place flow has been tested in a disposable hub area.

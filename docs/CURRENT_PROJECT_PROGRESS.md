...
- EarthLivingCore `0.7.3` is deployed on the hub test server with a placement preview command: `/el preview <width> <height> <depth> [yOffset] [seconds]`. It draws a temporary particle box so schematics can be height/footprint checked before Constructor or WorldEdit placement.

...
- ArchitectModule `0.1.0` has been implemented in the repository and deployed to the hub/test server as an admin-only blueprint/schematic generator. It provides `/architect search`, `/architect generate`, `/architect preview`, `/architect paste`, `/architect paste <id> look`, `/architect list` and `/architect reload`, writes `.schem` files through WorldEdit, keeps web/AI lookup optional and disabled by default, and loaded cleanly with WorldEdit, Citizens and Constructor on the hub. Main deployment should wait until one generated schematic has been tested with `/el preview look` and pasted in a disposable hub area.
